...
func _set_points() -> void:
	...
	for i in range(rays_amount + it):
		var point : Vector2 = _ray_to(
			to_global(Vector2(radius, 0).rotated(- _semiangle + i * _angle_step))
		)
...
func _check_collisions() -> void:
	for agent in _agents_in_reach_area.duplicate():
...
func reset_detection() -> void:
	_nodes_in_area_of_sight.clear()
	_agents_in_reach_area.clear()
